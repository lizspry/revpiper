...
`spec-*` files hold declaration surfaces: `spec-generic.R` (the shared
schema-driven check battery, named for its genericity over entry kinds —
renamed from spec-check.R, Liz 2026-07-12, since every spec file "checks";
the one named exception to files-as-workflow-units, as shared machinery
belongs to no single unit), `spec-source.R`
...
| 2026-07-19 | Pre-release code renumbering: with YX03 recoded YE10, the YX series closes its gap (old YX04 missing-joins -> YX03; old YX05 zero-dictionaries -> YX04). Never-reuse-codes begins at first release, not before | Liz: gaps protect released users; a private repo has none |
| 2026-07-19 | Check-scope taxonomy sharpened with the deletion test (could the check give the same verdict with every dictionary deleted? yes = not YX): YX03 recoded YE10 and moved to the standalone entry path (it now fires in single-file joins mode, which its cross-source placement wrongly prevented); YE09 confirmed entry-scope | Liz's challenge from the YE09 filing question |
| 2026-07-19 | Same-second report-folder collisions stay out of scope (re-affirmed over a review challenge): use is human-paced; a same-second audit+run of one spec set writes byte-identical reports (harmless); batch different-set scripting deferred until real | Liz |
| 2026-07-19 | joins = FALSE disposition stated wherever the summary appears — console, print(), and every report file: "joins excluded (joins = FALSE) and therefore not audited/run" | an excluded joins spec must never read as certified (review finding; wording Liz's) |
| 2026-07-19 | Adversarial battery round (blind pack + Liz's external-model run): empty/non-mapping spec files never certify (readers short-circuit only on parse error); YE09 self-joins illegal; YX04 zero dictionaries is a standing problem (no vacuous certification); report tree mirrors the spec folder with full input filenames (kills intra-run report collisions); five docs-precision fixes incl. scoping the related incomplete-search wording to the searched list | 39 internal + external oracle-first cases; 14 scenarios confirmed the contract, the rest converged on these decisions (Liz) |
...
| Provenance and audit trail across steps: certificates naming what they certified (immediate case: the spec certificate listing dictionaries by file + table name — incumbent for Task 17's annex), and more broadly how reports/certificates chain across steps (inputs named, runstamps linking a step's certificate to the artifacts the next step consumed; possibly content hashes) | Immediate bit: Task 17 walkthrough; the general model: Phase 2 (load) planning | Open (flagged 2026-07-12, Liz) |
| User-defined custom checks and validations: an extension point for users to add their own checks beyond the schema-generated battery (registration mechanism, codes, routing; precedents: the user-readers pattern — readers.R sourced into an isolated environment — and pointblank's `specially()` contract, a user fn returning a logical vector; 2026-07-12 review). BINDING requirement (Liz, 2026-07-12): reports and certificates must distinguish check provenance — engine-verified schema checks reported separately from author-supplied, unverified user checks (e.g. "34 schema checks (engine-verified); 6 user-defined checks (author-supplied, unverified)") — a certificate that silently mixes them is a certificate of mixed and undisclosed quality | Phase 3 (process) planning | Open (flagged 2026-07-12, Liz) |
| Additional check and type candidates (park list): circular between-field / self-reference chains beyond the existing YS04 level-nesting cycles and the acyclic-refers_to schema gap (§9); content types like DOI (maybe — Liz's caveat: strict format checks annoy users who are slightly off, e.g. a prefix; normalise-then-validate might mitigate); unicode-lookalike column names certify as distinct (battery hostile-18) — load-step normalisation territory | Phase 3 (process) planning; type candidates also at the usability pilot; lookalikes at Phase 2 (load) | Open (flagged 2026-07-12, Liz) |
| A simple data-overview report for eyeballing: per-table profile/summary to help users see their data and decide what checks it needs. Direction agreed (Liz, 2026-07-12): SPEC-AWARE (declared beside observed: types, value sets, missingness) and built in-house as a deliberately simple template — unless Phase 2 planning's scan finds something lightweight that does the job; pointblank's scan_data sections (overview, variables, interactions, correlations, missingness, sample) are the requirements starting point, its import stack the reason not to depend | Phase 2 (load) planning | Open (flagged 2026-07-12, Liz) |
...
- pointblank: rstudio/pointblank (July 2026 clone, v0.12.3.9000) —
  agent/interrogate model, action_levels thresholds, informant,
  scan_data, draft_validation, specially(), multiagent SHA1-keyed
  reports; full comparison in
  dev/superpowers/plans/2026-07-12-pointblank-review.md.
