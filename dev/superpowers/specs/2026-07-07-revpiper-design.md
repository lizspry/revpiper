...
| User-defined custom checks and validations: an extension point for users to add their own checks beyond the schema-generated battery (registration mechanism, codes, routing; precedent: the user-readers pattern — readers.R sourced into an isolated environment) | Phase 3 (process) planning | Open (flagged 2026-07-12, Liz) |
| A simple data-overview report for eyeballing: per-table profile/summary to help users see their data and decide what checks it needs (cf. pointblank's scan_data; 2026-07-12 review) | Phase 2 (load) planning | Open (flagged 2026-07-12, Liz) |
...
| Output locking, and living-review freezes: a way to mark an output frozen ("don't overwrite unless I say so") — especially a certified-and-locked dataset; per-publication / per-living-review-update snapshots that keep the published dataset frozen while re-runs for the next update diff against it. Interacts with the provenance/audit-trail question above and with lumberjack's change tracking | Later phase / user consultation (Liz raises "we need to talk about locking at some point") | Open (flagged 2026-07-12, Liz) |
