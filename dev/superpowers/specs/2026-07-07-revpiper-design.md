...
The schema itself: **one YAML file per input table** (`specs/tables/<table>.yaml`) carrying the table's `source:` block (file, sheet, reader), `description`, `roles:` (pipeline handles, one column per role; Phase 1 vocabulary: `study_id`), `levels:` (grouping name → key columns), and `columns:`. Per column: `type` — one of six plain types (`text`, `categorical` with required plain-list `values` whose entries' own type fixes the storage type, `integer`, `decimal`, `boolean`, `date`; no datetime in v1) — plus optional `range`/`units` (numerics/date), `pattern` (regex, key-format checks), `required`/`unique` booleans, `missing` codes (declared reactively via findings; only ""/whitespace are auto-missing), `constant_within_level`, `description`, and minimal composed keys (`from: [author, year]` + `separator` — concatenation only, because joins can't wait for the derive stage). The purpose/semantics axis (id/ordinal/quantity, value labels) is deliberately absent — designed later beside its consumers (derive, module 2). A closed schema with did-you-mean spec errors; parse-time spec validation is a named check layer. `rev_draft_dictionary()` (v1, minimal: column names + inferred types + empty fields) generates skeletons from data files; codebook-document parsing stays v1.x.
...
| Check-type→routing table for findings | Module-1 implementation planning | **Structured 2026-07-09** (five code families + consequence model, §3.7); full enumeration = Phase 1 plan deliverable |
