...
| Joins disposition in the spec step's audit/run commands: dict and joins specs may not both exist or be ready; incumbent three-state `joins` parameter (NULL auto / FALSE exclude / TRUE require) with the disposition always stated on the report + certificate (plan amendment 9d) | Task 17 walkthrough (Phase 1) | Open (flagged 2026-07-12) |
| Per-step `rev_<step>_run` vs the step-free `rev_run(through =)` (amendment 8): overlapping user surfaces; incumbent reconciliation reads `rev_<step>_run` as the stateless prefix run through that step, `rev_run()` reducing to sugar or retiring (plan amendment 9e) | Phase 2 planning | Open (flagged 2026-07-12) |
