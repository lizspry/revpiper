...
- **v2 execution amendment 6 (2026-07-08, signed off by Liz):** all GitHub Actions `uses:` references pinned to full commit SHAs (with tag comments), and `.github/dependabot.yml` added (weekly `github-actions` ecosystem) so pin updates arrive as automated PRs. Rationale: mutable tags are a supply-chain exposure (a retargeted tag executes attacker code with the run's token and secrets); SHA-pinning + Dependabot is the OpenSSF-recommended posture. Workflow logic untouched — only refs changed. CI R packages remain deliberately unpinned (conventions: renv pins the dev environment; CI tracks the ecosystem).
...
**Goal:** Turn the empty `revpiper` repo (GitHub-renamed from `review-pipeline`, 2026-07-07) into a green-CI R package skeleton (`revpiper`) with all conventions from spec §2 encoded in config files and documentation.
...
usethis::use_description(fields = list(
  ...
  URL = "https://github.com/lizspry/revpiper",
  BugReports = "https://github.com/lizspry/revpiper/issues",
  Depends = "R (>= 4.2)"
), check_name = FALSE)
...
```yaml
url: https://lizspry.github.io/revpiper/
template:
  bootstrap: 5
```
...
<!-- badges: start -->
[![R-CMD-check](https://github.com/lizspry/revpiper/actions/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/lizspry/revpiper/actions/workflows/R-CMD-check.yaml)
[![Codecov test coverage](https://codecov.io/gh/lizspry/revpiper/graph/badge.svg)](https://app.codecov.io/gh/lizspry/revpiper)
<!-- badges: end -->
...
- [ ] **Step 6 (LIZ, host): retire the folder-name mismatch.** After merge and once this sandbox is disposed of: rename the local working folder to `revpiper` (or make a fresh clone under that name and retire the old folder once satisfied). Create all future sandboxes from the renamed folder so `usethis::create_package`-style tooling never hits the invalid-name workaround again (amendment 4).
- [ ] **Step 7: exit checklist.** CI green on main; site live; badges render; branch protection intact; local folder renamed (Step 6). Then Phase 1 planning (opens with the extraction-tool export investigation + data-dict format decision, spec §8.2/§8.4; also queued from Phase 0: specify which top-level `.md` files pkgdown renders into the site — the default renders them all, so the built site currently includes an unlinked `CLAUDE.html` from CLAUDE.md; public-repo content, but the rendered set should be chosen, not defaulted — logged 2026-07-07).
